...
test("activation preserves other apps and other VOLT/16 installations on the same origin", async () => {
  ...
  await activation;
  assert.deepEqual(removed, [`volt16:${scope}:shell-v5`, `volt16:${scope}:shell-v6`, `volt16:${scope}:shell-v7`, `volt16:${scope}:shell-v8`, `volt16:${scope}:shell-v9`, `volt16:${scope}:shell-v10`, `volt16:${scope}:shell-v11`, `volt16:${scope}:shell-v12`, `volt16:${scope}:shell-v13`, `volt16:${scope}:shell-v14`, `volt16:${scope}:shell-v15`]);
});
...
test("offline requests read only this installation's cache", async () => {
  ...
  assert.deepEqual(opened, [`volt16:${scope}:shell-v16`]);
});

test("visible version, application script, and service-worker shell use r13.1.1 consistently", () => {
  assert.match(indexSource, /VOLT\/16 · AUDIO r13\.1\.1/);
  assert.match(indexSource, /<script src="\.\/app\.js\?v=16"><\/script>/);
  assert.match(source, /shell-v16/);
