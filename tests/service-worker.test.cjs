...
test("activation preserves other apps and other VOLT/16 installations on the same origin", async () => {
  ...
  await activation;
  assert.deepEqual(removed, [`volt16:${scope}:shell-v5`, `volt16:${scope}:shell-v6`, `volt16:${scope}:shell-v7`, `volt16:${scope}:shell-v8`, `volt16:${scope}:shell-v9`, `volt16:${scope}:shell-v10`]);
});
...
test("offline requests read only this installation's cache", async () => {
  ...
  assert.deepEqual(opened, [`volt16:${scope}:shell-v11`]);
});

test("visible version, application script, and service-worker shell use r11 consistently", () => {
  assert.match(indexSource, /VOLT\/16 · AUDIO r11/);
  assert.match(indexSource, /<script src="\.\/app\.js\?v=11"><\/script>/);
  assert.match(source, /shell-v11/);
